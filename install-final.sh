...
case "${1:-start}" in
    ...
    update)
        echo "更新Edge Worker..."
        tmp_install="$(mktemp)"
        curl -sSL "https://raw.githubusercontent.com/Charles-beta-he/hermes-edge-worker/main/install-final.sh" -o "$tmp_install" || { echo "更新脚本下载失败；如为 TLS 证书问题，请先修复 CA/代理证书，或手动运行 HERMES_EDGE_ALLOW_INSECURE_SSL=1 bash install-final.sh"; rm -f "$tmp_install"; exit 1; }
        bash "$tmp_install"
        rm -f "$tmp_install"
        ;;
    *)
