...
class TaskEventDriven:
    ...
    def __init__(self, event_bus=None, task_pool=None, orchestrator=None):
    ...
    def auto_execute(self, task_id: str):
        ...
        try:
            # 执行任务
            if self.orchestrator:
                result = self.orchestrator.autorun_runner(task_id)
            ...
            def do_POST(self):
                ...
                if self.path == "/event":
                    try:
                        ...
                        # 处理事件
                        self.event_driven.handle_event(event_type, event_data)
